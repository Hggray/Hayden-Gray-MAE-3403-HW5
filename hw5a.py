...
def ff(Re, rr, CBEQN=False):
    ...
    if(CBEQN):
        ...
        result= fsolve(cb, 0.01)  # use fsolve to find the friction factor using Colebrook equation
        return result[0]
    else:
        return 64/Re
...
def plotMoody(plotPoint=False, pt=(0,0)):
    ...
    ReValsCB=np.logspace(np.log10(4000), np.log10(1000000000), 100)# for use with Colebrook equation (i.e., Re in range from 4000 to 10^8)
    ReValsL=np.logspace(np.log10(600.0),np.log10(2000.0),20)  # for use with Laminar flow (i.e., Re in range from 600 to 2000)
    ReValsTrans=np.logspace(np.log10(2000), np.log10(4000), 100)  # for use with Transition flow (i.e., Re in range from 2000 to 4000)
    ...
    ffTrans=np.array([ff(Re, 0, True) for Re in ReValsTrans]) # use list comprehension for all Re in ReValsTrans and calling ff
    ...
    plt.loglog(ReValsTrans, ffTrans, 'r--', label='Transition')  # plot the transition part as a dashed line
    for nRelR in range(len(ffCB)):
        ...
        plt.annotate(xy=(float(ReValsCB[-1]), float(ffCB[nRelR, -1])),text=str(rrVals[nRelR]))  # put a label at end of each curve on the right
